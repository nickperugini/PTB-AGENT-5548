...
resource "aws_apigatewayv2_stage" "api1_stage" {
  ...
  access_log_settings {
    destination_arn = aws_cloudwatch_log_group.api1_logs.arn
    format          = jsonencode({
...
resource "aws_cloudwatch_log_group" "api1_logs" {
  name              = "/aws/apigateway/api1"
  retention_in_days = 7
  # Generated by AIG Copilot
}

resource "aws_cloudwatch_log_group" "api2_logs" {
  name              = "/aws/apigateway/api2"
  retention_in_days = 7
  # Generated by AIG Copilot
}

# Example: Add access_log_settings to your API Gateway stages
resource "aws_apigatewayv2_stage" "api1_stage" {
  api_id      = aws_apigatewayv2_api.api1.id
  name        = "$default"
  auto_deploy = true
